...
import vn.co.honda.hondacrm.btu.Manager.PhoneStateManager;
import vn.co.honda.hondacrm.btu.Model.control.VehicleMessageSabFlag;
import vn.co.honda.hondacrm.btu.Model.message.notification.VehicleNotificationMessage;
...
//        BluetoothManager.getInstance().getBleConnectController().setVehicleConnectListener(this);
        ...
        if (isRegistered) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                channelId = createNotificationChannel("connmc_id", "My Foreground Service");
            } else {
...
//                break;
                    ...
                    case ConnMCService.PACKAGE_NAME_WHATSTAPP_BUSSINESS:
                        mNotificationList.put(packetName, count);
                        message = "WHATSAPP" + String.format("%03d", count);
                        break;
    ...
    private void resolveNoticeContent(String message) {
        ...
        if (isRegistered) {
            ...
            // Send config SAB FLAG ON
            VehicleMessageSabFlag vehicleMessageSabFlag = new VehicleMessageSabFlag(VehicleMessageSabFlag.SAB_FLAG_ON);
            BluetoothManager.getInstance().sendMessage(vehicleMessageSabFlag);

            //Create VehicleInformation Transaction Request Message.
